...
PDF 中展示了大量具体的数学教育数字工具，分为几大类。

### 动态几何与代数工具
- GeoGebra：集成动态几何、函数图像、电子表格、CAS、3D 的免费开源平台（详见下文）
- Grid Algebra：用网格可视化代数运算，学生通过路径探索体验代数结构。来源：Hewitt, D. (2016) "Designing Educational Software: The Case of Grid Algebra", Digital Experiences in Mathematics Education 2, 167-198
- Geometry Expressions (GXWeb)：几何作图与符号计算结合的工具，支持 draw, define, discover 的探索流程
- Numworx (Freudenthal Institute)：基于 RME 理念的数字数学环境，包含 area algebra、brackets and tables 等交互练习
- Digital Mathematics Environment (DWO)：Freudenthal Institute (Utrecht University) 开发的数字学习环境，特点是基于用户反馈和研究持续开发、teacher as author（教师可编辑和设计模块）

### 数据分析与统计工具
- CODAP (Common Online Data Analysis Platform)：面向教育的数据分析软件，继承了 Fathom 和 TinkerPlots 的传统。表征之间动态链接（表格、图形、地图中选中的数据项自动在其他表征中高亮）

### 模拟与互动工具
- PhET Interactive Simulations：University of Colorado 开发的互动模拟，数学相关的包括 Unit Rates、Make a Ten、Plinko Probability、Function Builder、Trig Tour、Arithmetic
- Polypad at Mathigon：提供多种虚拟操作物（如 Exponential Decay with Dice、Factoring Quadratics Practice、Galton Board、Decimal Multiplication、Equality with Number Bars、Exploring the Pythagorean Theorem）
- TouchCounts：幼儿用手指和耳朵学习计数、加减法的应用，包括 Numbers World（建立 number sequence, enumeration, ordinality）和 Operations World（通过推拼数字理解 cardinality, addition, subtraction）

### 代数思维工具
- SolveMe Puzzles：代数学习的拼图游戏（Mobiles, Who Am I?, MysteryGrid），强调 explore, deduce, create
- Graspable Math：通过手势操作进行代数证明的工具
- Center for Algebraic Thinking：包含 Tortoise & The Hare（代数竞赛）、Hop the Number Line、cover-up equation solving 等活动
- CandyFactory Educational Game：通过糖果工厂场景学习分数的教育游戏

### 几何探索工具
- Geometry for Young Learners (SFU)：基于 Web Sketchpad 的幼儿几何活动集，包括 Tessellations、Hidden Paths、Line Tools、Spirolateral、Circle Party、Triangle Jam
- Technologically Embodied Geometric Functions：通过感觉运动经验发展抽象函数概念的方法，基于 mathematics, cognitive science, technology, pedagogy 四个基础
- Dynamic Number / Sketchpad Explorer：iPad 上的数学探索应用
- Cabri Express / Cabricloud：免费的综合数学应用

### 课程资源平台
- NCTM illuminations：美国数学教师协会的在线活动和课程计划资源
- Math Learning Center：基于 Bridges in Mathematics 课程的视觉模型应用，提供 web app 和移动版

## 核心内容 4：Dynamic Geometry 和 GeoGebra
...
## 核心内容 6：Computational Thinking 和 Coding
...
- Hewitt, D. (2016)：Grid Algebra 的教育软件设计，Digital Experiences in Mathematics Education 2, 167-198
- Sinclair, N. et al.：TouchCounts, Geometry for Young Learners (SFU)
...
### Readings
- [[R01-Bruner|R01 Bruner]] — Bruner 的 enactive-iconic-symbolic 表征理论与数字工具的多重表征功能直接对应，GeoGebra 等工具同时展示多种表征并动态链接
- [[R05-RME2026|R05 RME]] — Numworx 和 Digital Mathematics Environment (DWO) 均来自 Freudenthal Institute，体现了 RME 的 guided reinvention 理念在数字环境中的实现
- [[R06-Teaching-with-Variation|R06 Variation]] — GeoGebra 的 dragging 功能是实现 variation theory 中 dimensions of variation 和 range of change 的理想工具
- [[R11-Modelling-in-Classroom|R11 Hall & Lingefjard]] — Hall & Lingefjard 全章围绕 GeoGebra 的课堂使用
- [[R13-RME-Emergent-Modelling|R13 Emergent Modelling]] — Numworx 的 area algebra 等工具体现了 emergent modelling 在数字环境中的应用

### Cases
- [[C7 Grid Algebra]] — Hewitt (2016) 的 Grid Algebra 在本讲 PDF 中作为教育软件设计的案例展示
- [[C8 Class Activities (WODB + PhET)|C8 Class Activities]] — PhET 的 Function Builder 等模拟在本讲中作为互动数学工具的典型示例
